...
- **深浅色主题** — 三套内置主题跑在严格 17-token 主题引擎上：终端深色（默认）、纸感浅色、高对比深色。每套主题（含浅色）的对比度都在测试里按 WCAG AA 逐项校验；原生控件、滚动条、应用图标都会跟随当前深浅方案。另有字号调节，全局与转录可分开设置。
- **免账号历史** — 全部存浏览器 IndexedDB，支持搜索曾出现过的表达；设置里可导出全量备份、导入恢复。
