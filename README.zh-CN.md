...
<picture>
  ...
  <img src="public/icon-ui-light.png" width="150" alt="JargonSlayer 图标" />
</picture>
...
- **实时转录** — 浏览器识别（云端）/ 本地 Whisper / 标签页音频（后两者音频不出本机），每个引擎带「本地 / 云端」数据去向标识；内置演示零依赖看完整流程。浏览器识别加了语音活动检测兜底：分段切换只挑真正的停顿点下手，断线恢复不会丢掉已经缓存的语音，识别语种持续对不上时只提醒一次，不会悄悄丢字。
...
（`--partials` 让说话过程中先出灰色的中间结果，也就是打字机效果；机器吃紧可以去掉。）等横幅打印出来再回网页操作。第一次运行会先从 Hugging Face 下载模型，端口要等下载和模型加载都完成才会打开。视模型大小和网速，这一步可能要等一会儿才有任何输出——是正常现象，不是卡住了。大概会看到这样的内容：
...
**该选哪个引擎**：只用麦克风 → 本地 Whisper。要听线上会议对方的声音，或者抓任意标签页/流媒体 → 标签页音频（同一个 sidecar，照样不出本机）。什么都不想装 → 浏览器识别，零配置兜底方案——但话说清楚：这条路音频会送到 Chrome 的语音服务（Google）。
...
跑起来之后常用参数：`--language en`（默认）、`--partials`（说话过程中出灰色中间结果，更费 CPU）、`--save-audio meeting.wav`（保留录音，供会后说话人分离）。
