...
// manifest. A second host blocker hides behind that one: the
// `../.tauri/tauri-api` path dependency below is generated by
// `tauri ios build` and gitignored, so on a fresh checkout `swift test`
// fails at dependency RESOLUTION before the UIKit compile error is even
// reachable. Run scripts/test-ios-core.mjs instead (wired into ci.yml's
// native job): it stages Sources/OsSpeechCore + Tests/OsSpeechCoreTests
// into a scratch package with no Tauri dependency and `swift test`s
// there — the automated form of the manual drop-the-glue-target
// workaround this comment used to prescribe. CI/lane gates for the REAL
// iOS target are unaffected: `xcodebuild`/`swift build --triple
// arm64-apple-ios` build the whole package, including the glue target,
