...
// Linear amplitude floor a windowed peak must clear to count as "real
// speech-level audio", not noise-floor hiss — measured room floor is
// 0.022 (docs/design-explorations/dual-capture-2026-08.md:57); this adds
// margin.
export const AUDIO_FLOOR = 0.03;
 ...
 *  (pushAudioWindow, noteResult) AND on the sentinel's own tick, so no
 *  two call sites can ever disagree about what counts as active. */
function computeVerdicts(channels: Record<AudioChannel, ChannelLivenessState>, now: number): AudioLivenessVerdicts {
...
export function pushAudioWindow(channel: AudioChannel, windowPeak: number): void {
  const now = Date.now();
...
function evaluateSentinel(): void {
  useAudioLiveness.setState((s) => {
    if (!s.armed) return s;
    return { verdicts: computeVerdicts(s.channels, Date.now()) };
  });
}
