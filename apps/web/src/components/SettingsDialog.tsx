          ...
          <section className="space-y-3" data-ui-level="engine">
            ...
            <div className="grid grid-cols-2 gap-2">
              {ENGINE_CARDS.map((opt) => {
                ...
                // kind serves the current source. Post-merge with the
                // paired-write round (fb6e415) every card click STAGES
                // mode into the draft, so the draft is the in-dialog
                // truth: resolve from draft.mode (click 系统识别 then
                // 本地模型 must yield appaudio, the staged system-audio
                // lane — live settings.mode would still say the
                // pre-dialog source). Every other card's gate/retention/
                // click below keeps reading opt.value unchanged via this
                // same fallback.
                const resolvedSidecarKind = opt.sidecarFamily
                  ? sidecarEngineForMode(draft.mode)
                  : undefined;
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      // next launch's migrateSettings heal. Merged with
                      // the taxonomy round: the staged engine is the
                      // RESOLVED kind (gateValue — the 本地模型 card's
                      // sidecar lane, opt.value for every other card).
                      // import/url are standalone intent, never
                      // clobbered by an engine pick (store.ts
                      // isModeLegalForPlatform's "always fine to keep"
                      // ruling).
