...
> **语言:** [English](../PACKAGING.md) · 简体中文（英文版为权威原文，不同步时以英文版为准）
...
- 注意：PWA 依然需要 Next.js 服务在某处运行（本机或局域网内一台机器）。`npm start` 常驻即可。
...
2. Next.js 以 **sidecar 进程**方式打包：`tauri.conf.json` 的 `externalBin` 带上 `next start` 的 standalone 输出（`next.config.mjs` 加 `output: "standalone"`），Tauri 启动时拉起本地 3000 端口再加载 `http://localhost:3000`；
...
- `src/app/icon.svg`：产品签名图标（深色转录行 + 金色高亮表达 + 虚线下划线），Next.js 自动作为 favicon；
