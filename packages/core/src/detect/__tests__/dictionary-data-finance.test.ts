...
const MY_SURFACES = candidateSurfaces(FINANCE_CONSUMER_TERMS, FINANCE_CONSUMER_EXPRESSIONS);

describe("FINANCE_CONSUMER_TERMS — data integrity", () => {
  it("every entry has non-empty required fields, a valid TermType, and pack 'finance-consumer'", () => {
    expect(FINANCE_CONSUMER_TERMS.length).toBeGreaterThan(0);
    for (const entry of FINANCE_CONSUMER_TERMS) {
      ...
      expect(entry.pack).toBe("finance-consumer");
    }
  });
...
describe("FINANCE_CONSUMER_EXPRESSIONS — data integrity", () => {
  it("every entry has non-empty required fields, a valid ExpressionCategory, confidence 0.9, and pack 'finance-consumer'", () => {
    expect(FINANCE_CONSUMER_EXPRESSIONS.length).toBeGreaterThan(0);
    for (const entry of FINANCE_CONSUMER_EXPRESSIONS) {
...
describe("entry count is roughly the ~60 the brief asked for", () => {
  it("terms + expressions together land in a sane range around 60", () => {
    const total = FINANCE_CONSUMER_TERMS.length + FINANCE_CONSUMER_EXPRESSIONS.length;
...
describe("no duplicate surfaces", () => {
  it("no duplicate surface within FINANCE_CONSUMER_TERMS + FINANCE_CONSUMER_EXPRESSIONS themselves", () => {
  ...
  it("no surface collides with the sibling v0.6 packs (modern-usage, daily-idiom)", () => {
    ...
    for (const s of MY_SURFACES) {
      expect(siblingSurfaces.has(norm(s)), `"${s}" collides with modern-usage/daily-idiom`).toBe(false);
    }
  });
  ...
  it("no surface is matched by the LIVE built-in dictionary (base tables + extra + compiled, via scanDictionary)", () => {
    ...
    // BASE_EXPRESSIONS/BASE_TERM_DICTIONARY (e.g. "runway", "cap table",
    // "burn rate" — already covered under other packs) without editing
...
describe("commonWord — hand-listed subset (pins intent, not just current data)", () => {
  const EXPECTED_COMMON = ["premium", "principal", "collateral", "dilution", "withholding", "itemize"];

  it("flags exactly the expected everyday-English headwords", () => {
    const flagged = FINANCE_CONSUMER_TERMS.filter((t) => t.commonWord).map((t) => t.term).sort();
    expect(flagged).toEqual([...EXPECTED_COMMON].sort());
  });

  it("does NOT flag finance-only vocabulary that has no everyday meaning", () => {
    for (const term of ["deductible", "coinsurance", "RSU", "escrow", "amortization", "COBRA"]) {
      const entry = FINANCE_CONSUMER_TERMS.find((t) => t.term === term);
...
describe("PRC financial terminology spot-check", () => {
  it("uses the standard PRC term for the brief's own worked examples", () => {
    const deductible = FINANCE_CONSUMER_TERMS.find((t) => t.term === "deductible");
    const copay = FINANCE_CONSUMER_TERMS.find((t) => t.term === "copay");
    const vesting = FINANCE_CONSUMER_TERMS.find((t) => t.term === "vesting");
    const dilution = FINANCE_CONSUMER_TERMS.find((t) => t.term === "dilution");
    expect(deductible?.gloss_zh).toContain("免赔额");
    expect(copay?.gloss_zh).toContain("共付额");
    expect(vesting?.gloss_zh).toContain("归属");
    expect(dilution?.gloss_zh).toContain("稀释");
  });
});
