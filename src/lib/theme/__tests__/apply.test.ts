...
import { activateTheme, applyTheme, resetToDefaultTheme } from "../apply";
...
describe("applyTheme", () => {
  it("sets every token via CSSOM setProperty, not a <style> string", () => {
  ...
  it("overwrites a previously-applied theme's tokens", () => {
    applyTheme("clarity", CLARITY_THEME.tokens);
...
describe("resetToDefaultTheme", () => {
  it("removes every inline token override via removeProperty", () => {
    applyTheme("clarity", CLARITY_THEME.tokens);
    for (const key of THEME_TOKEN_KEYS) {
...
describe("activateTheme", () => {
  ...
  it("routes any non-terminal id through applyTheme", () => {
